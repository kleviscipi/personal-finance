...
const form = useForm({
    ...
    currency: props.transaction?.currency || props.currentAccount?.base_currency || 'USD',
    date: '',
    category_id: props.transaction?.category_id || '',
    subcategory_id: props.transaction?.subcategory_id || '',
    description: props.transaction?.description || '',
    payment_method: props.transaction?.payment_method || 'cash',
    tag_ids: props.transaction?.tags?.map((tag) => tag.id) || [],
    tag_names: '',
    tag_list: (props.transaction?.tags || []).map((tag) => tag.name),
});
...
form.date = normalizeDateInput(props.transaction?.date);
